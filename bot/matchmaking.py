...
from salute.giga import analyze_winner, evaluate_player_performance
...
async def process_completed_match(session: AsyncSession, slot: RoomSlot):
    ...
    try:
        ...
        # Персональные оценки игроков с извлечением краткого анализа
        player1_evaluation = await evaluate_player_performance(
            transcription_text, case_context, slot.player1.full_name
        )
        player1_evaluation_text = player1_evaluation.get('answer', '')
        
        # Извлекаем баллы
        points_match = re.search(r"Общая оценка за раунд:\s*(\d+)", player1_evaluation_text)
        slot.player1_points = int(points_match.group(1)) if points_match else 0
        slot.player1.sum_points += slot.player1_points
        
        # Извлекаем краткий анализ выступления
        analysis_match = re.search(r"Краткий анализ выступления:\s*(.+)", player1_evaluation_text, re.DOTALL)
        slot.player1_analysis = analysis_match.group(1).strip() if analysis_match else "Анализ недоступен"

        player2_evaluation = await evaluate_player_performance(
            transcription_text, case_context, slot.player2.full_name
        )
        player2_evaluation_text = player2_evaluation.get('answer', '')
        
        points_match = re.search(r"Общая оценка за раунд:\s*(\d+)", player2_evaluation_text)
        slot.player2_points = int(points_match.group(1)) if points_match else 0
        slot.player2.sum_points += slot.player2_points
        
        analysis_match = re.search(r"Краткий анализ выступления:\s*(.+)", player2_evaluation_text, re.DOTALL)
        slot.player2_analysis = analysis_match.group(1).strip() if analysis_match else "Анализ недоступен"
        ...
        # НОВАЯ ЛОГИКА: Проверка на минимальное количество баллов
        # Проверяем, что оба балла не None и оба меньше порога
        if (slot.player1_points is not None and slot.player2_points is not None and
            slot.player1_points < MIN_POINTS_TO_WIN and slot.player2_points < MIN_POINTS_TO_WIN):
            # Оба игрока получают меньше минимального порога - оба проигрывают
            slot.first_is_winner = None
            if elimination:
                slot.player1.eliminated = True
                slot.player2.eliminated = True
        else:
            # СТАРАЯ ЛОГИКА: Определение победителя обычным способом
            # Проверка и корректировка победителя на основе баллов
            if slot.first_is_winner is None:
                # Если не удалось определить победителя по вердикту, используем баллы
                if slot.player1_points != slot.player2_points:
                    slot.first_is_winner = slot.player1_points > slot.player2_points
                else:
                    # Если баллы равны, определяем победителя по длине текста
                    if player1_text_length < player2_text_length:
                        slot.first_is_winner = True
                    elif player1_text_length > player2_text_length:
                        slot.first_is_winner = False
                    # Если длины текстов одинаковые, оставляем победителя None (оба проходят)
            else:
                # Если победитель определен по вердикту, но баллы сильно отличаются в пользу другого игрока
                if slot.first_is_winner and (slot.player2_points - slot.player1_points) > significant_difference:
                    slot.first_is_winner = False
                elif not slot.first_is_winner and (slot.player1_points - slot.player2_points) > significant_difference:
                    slot.first_is_winner = True
        ...
        logging.info(f"Матч {slot.id} обработан. Победитель: {slot.first_is_winner}")
...
async def calculate_player_text_length(transcription: str, player_name: str) -> int:
    ...
    try:
        ...
        logging.error(f"Ошибка при подсчете длины текста для игрока {player_name}: {e}")
        return 0
